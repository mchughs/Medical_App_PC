...
public class Medicine {
    ...
    public LocalDate recieved_date;
    ...
    LocalDate expiration, LocalDate recieved_date) {
        this.name          = name;
        this.count         = count;
    ...
    public void printMedicineComplete() {
        ...
        System.out.println("Recieved:     " + recieved_date.toString());
        checkReplace();
    }
    ...
    public void checkReplace() {
      if(currDate.compareTo(expiration) > 0) { // Expired
      ...
      } else if(count - (ChronoUnit.WEEKS.between(recieved_date, currDate)*frequency) <= 0) { // Out of
        replace = true;
        ...
        if(frequency == 0) {
          ...
          if(count % frequency == 0) {
            outDate = recieved_date.plus((count/frequency), ChronoUnit.WEEKS);
          } else {
            outDate = recieved_date.plus(1 + (count/frequency), ChronoUnit.WEEKS);
          }
